...
fn should_not_pass_update() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            DidModule::update(Origin::signed(4), 5u64),
            Error::<Test>::DidNotExists
        );

        assert_noop!(
            DidModule::update(Origin::signed(2), 3u64),
            Error::<Test>::PublicKeyUsed
        );
    });
}
...
fn without_did_should_not_pass_lock() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            DidModule::lock(Origin::signed(1), 100, 5),
            Error::<Test>::DidNotExists
        );
        ...
        assert_noop!(
            DidModule::lock(Origin::signed(4), 100, 5),
            Error::<Test>::SuperiorNotExists
        );
    });
}
...
fn should_not_pass_unlock() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            DidModule::unlock(Origin::signed(2), 2000),
            Error::<Test>::UnreservedFundsExceed
        );

        assert_ok!(Balances::reserve(&3, 100));

        assert_noop!(
            DidModule::unlock(Origin::signed(3), 10),
            Error::<Test>::NotLockFunds
        );

        assert_ok!(Balances::reserve(&4, 100));

        assert_noop!(
            DidModule::unlock(Origin::signed(4), 10),
            Error::<Test>::DidNotExists
        );
        ...
        assert_noop!(
            DidModule::unlock(Origin::signed(2), 10),
            Error::<Test>::UnlockTimeNotReach
        );
    });
}
...
fn should_not_pass_transfer() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            DidModule::transfer(Origin::signed(4), user_key_1, 100, memo.to_vec()),
            Error::<Test>::DidNotExists
        );

        assert_noop!(
            DidModule::transfer(Origin::signed(2), H256::zero(), 100, memo.to_vec()),
            Error::<Test>::DidNotExists
        );

        assert_noop!(
            DidModule::transfer(Origin::signed(1), user_key_1, 100, memo.to_vec()),
            Error::<Test>::SentToSelf
        );

        assert_noop!(
            DidModule::transfer(Origin::signed(3), user_key_1, 10001, memo.to_vec()),
        ...
        assert_noop!(
            DidModule::transfer(Origin::signed(1), user_key_4, 1000, memo.to_vec()),
            Error::<Test>::SuperiorNotExists
        );
    });
}
...
fn should_pass_add_external_address() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(DidModule::add_external_address(
            Origin::signed(1),
            b"eos".to_vec(),
            EOS_ADDRESS.to_vec()
        ));
...
fn should_pass_set_group_name() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(DidModule::set_group_name(
            Origin::signed(2),
            b"btc group".to_vec()
        ));
    });
